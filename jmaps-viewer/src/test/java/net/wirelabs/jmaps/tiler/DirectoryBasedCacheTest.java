...
class DirectoryBasedCacheTest {
    ...
    void testCacheKeyExists() {
        cache.put(URL2, TEST_IMAGE);
        Assertions.assertThat(cache.contains(EXPECTED_XYZ_FILE2.toPath().toString())).isTrue();
    }
    ...
    void getExistingFromCache() {
        cache.put(URL3, TEST_IMAGE);
        Assertions.assertThat(cache.get(EXPECTED_CACHE_KEY3)).isNotNull();
    }
    @Test
    void getNonExisting() {
        Assertions.assertThat(cache.get("nonexisting")).isNull();
    }
}
